                ...
                if len(columns) > 2 and columns[0].text.strip().startswith("AS"):
                    if columns[2].text.strip() != "0":
                        selected_data.append(
                            columns[0].text.strip().replace("AS", "IP-ASN,")
                        )
...
def write_data_to_file(region_code, selected_data, url):
    try:
        if len(selected_data) > 100:
            ...
            print(
                f"{region_code}ASN Generated from {url} and saved to ASN{region_code}.list."
            )
            ...
            print(
                f"{region_code}ASN Generated from {url} sorted and saved to A{region_code}.list."
            )
        else:
            print(
                f"Result has less than 100 lines for {region_code}. Not writing to file."
            )
    except IOError as e:
        print(
...
def scrape_data():
    try:
        for region_code in region_and_flag.keys():
            ...
            time.sleep(
                random.uniform(2, 5)
            )  # delay for a random number of seconds between * and *
    except Exception as e:
